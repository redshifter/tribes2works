...
datablock PlayerData(LightMaleHumanArmor) : LightPlayerDamageProfile
{
   ...
   runEnergyDrain = 0;
   minRunEnergy = 0;
   maxForwardSpeed = 30;  // HYPERDRIVE double speed
   maxBackwardSpeed = 26; // HYPERDRIVE double speed
   maxSideSpeed = 26;     // HYPERDRIVE double speed
   ...
   minImpactSpeed = 45 * 2;       // HYPERDRIVE double impact speed
   speedDamageScale = 0.0022 / 2; // HYPERDRIVE halved damage scale
...
function Armor::onCollision(%this,%obj,%col,%forceVehicleNode)
{
   ...
   if (%forceVehicleNode !$= "" || (%className $= WheeledVehicleData || %className $= FlyingVehicleData || %className $= HoverVehicleData) &&
      ...
      if(%col.getState() $= "Dead") {
         ...
         // ammo (cannot pick up ammo from a dead body)
...
//error("Armor::damageObject( "@%data@", "@%targetObject@", "@%sourceObject@", "@%position@", "@%amount@", "@%damageType@", "@%momVec@" )");
  ...
  // overdrive disc doesn't hurt you
   if(%targetObject == %sourceObject && %damageType == $DamageType::Disc) {
	   return;
   }
