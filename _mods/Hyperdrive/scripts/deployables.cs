...
$TurretIndoorSpaceRadius  = 18;  // deployed turrets must be this many meters apart // z0dd - ZOD, 8/14/02. Was 20
...
$TurretOutdoorSpaceRadius  = 20;  // deployed turrets must be this many meters apart // z0dd - ZOD, 8/14/02. Was 25
...
//$NotDeployableReason::OrganicTooClose           =  12; // z0dd - ZOD, 4/18/02. Address deploy of objects inside other objects.
...
datablock AudioProfile(SensorDeploySound)
{
   ...
   //effect = MotionSensorDeployEffect; // z0dd - ZOD 6/21/02. From Durt, wrong place
};
...
datablock AudioProfile(MotionSensorDeploySound)
{
   ...
   description = AudioClose3d;
   preload = true;
   effect = MotionSensorDeployEffect; // z0dd - ZOD 6/21/02. Was in wrong place
};
...
//------------------------------------------------------------------------------------
// z0dd - ZOD - BadShot - Founder, 6/24/02. Address deploying of objects inside other 
// objects. New function.
//function ShapeBaseImageData::testOrganicTooClose(%item, %plyr)
//{
//   InitContainerRadiusSearch( %item.surfacePt, $MaxDeployableDistance, $TypeMasks::StaticTSObjectType );
//   %test = containerSearchNext();
//   if(%test)
//   {
//      for (%i = 0; %i < $NumStaticTSObjects; %i++)
//      {
//         if(getWord($StaticTSObjects[%i], 2) $= %test.shapeName)
//         {
//            %organicIndex = %i;
//            break;
//         }
//      }
//      if(getWord($StaticTSObjects[%organicIndex], 0) $= Organics)
//      {
//         %scale1 = getWord(%test.scale, 1);
//         %scale2 = getWord(%test.scale, 2);
//         %maxDist1 = %scale1 > 1 ? 3.2 * %scale1 : "";
//         %maxDist2 = %scale2 > 1 ? 3.2 * %scale2 : "";
//         %maxDist = %maxDist1 >= %maxDist2 ? %maxDist1 : %maxDist2;
//         %maxDist = %maxDist $= "" ? 3.2 : %maxDist;
//         %distance = VectorDist(getWords(%item.surfacePt, 0, 1) SPC 0, getWords(%test.getWorldBoxCenter(), 0, 1) SPC 0);
//         if (%distance <= %maxDist)
//            return %test;
//         else
//            return false;
...
{
   if(%data.turretBarrel !$= "")
   {
      // z0dd - ZOD, 5/18/03. This is for turret barrel packs, to exec the barrel swap code, streamline.
...
// z0dd - ZOD, 5/18/03. New default parent deactivate function, streamline.
...
// -----------------------------------------------------------------------------------
// z0dd - ZOD, 12/13/02. Fixes not being able to use deployable inv after deploying it
//                       without stepping out of it's collision box. New function.
