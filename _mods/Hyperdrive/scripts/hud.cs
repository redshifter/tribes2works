...
{
   if(%client.isGagged) // z0dd - ZOD - Founder, 7/13/03. Check for admin gag
      return;

   // TracerDX/ilys - Server crash exploit fix
   if((%client.voice $= "") && (strstr(%text, "~w") != -1) && (getSubStr(%text, (strlen(%text) - 4), 4) !$= ".wav"))
      return;

   if( strstr(%text, "\n") != -1 ) // z0dd - ZOD, 6/7/03. Disallow \n chat spam
   {
      //%text = stripchars(%text, "\n");
      messageClient(%client, 'MsgYouSuck', '\c2Newline characters are not allowed in chat, llama.');
      logEcho(%client.nameBase @ " tried to spam the server with newlines on teamchat.");	
      return;
   }
   if( %client.voice $="" && $Host::ClassicNoNullVoiceSpam ) // z0dd - ZOD, 8/9/03. Disallow null voice chat spam
   {
      //%wavStart = strstr( %text, "~w" );
      //%text = getSubStr(%message, 0, %wavStart);
      messageClient(%client, 'MsgYouSuck', '\c2NULL voice exploit is not allowed on this server, llama.');
      logEcho(%client.nameBase @ " tried to spam the server with null voice exploit.");
      return;
   }
   if(strlen(%text) >= $Host::MaxMessageLen)
      %text = getSubStr(%text, 0, $Host::MaxMessageLen);

   if($Host::ClassicBadWordFilter) // z0dd - ZOD, 6/7/03. Allow admins to control language on server
...
function serverCmdMessageSent(%client, %text)
{
   ...
   if( strstr(%text, "\n") != -1 ) // z0dd - ZOD, 6/7/03. Disallow \n chat spam
   {
      //%text = stripchars(%text, "\n");
      messageClient(%client, 'MsgYouSuck', '\c2Newline characters are not allowed in chat, llama.');
      logEcho(%client.nameBase @ " tried to spam the server with newlines on global.");	
      return;
   }
   if( %client.voice $="" && $Host::ClassicNoNullVoiceSpam ) // z0dd - ZOD, 8/9/03. Disallow null voice chat spam
   {
