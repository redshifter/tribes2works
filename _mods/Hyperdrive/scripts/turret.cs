...
// z0dd - ZOD, 5/18/03. New base turret hum sound efx
datablock AudioProfile(BaseTurretHumSound)
{
...
// z0dd - ZOD, 5/18/03. New deployable turret hum sound efx
datablock AudioProfile(DeployedTurretHumSound)
{
...
{
   // z0dd - ZOD, 8/9/03. Turrets disabled in Spawn CTF.
   if(isObject(Game))
   {
   ...
   // z0dd - ZOD, 5/05/04. Allow bot controlled vehicle turrets to fire on anything..
...
function TurretData::replaceCallback(%this, %turret, %engineer)
{
   ...
   if (%engineer.getMountedImage($BackPackSlot) != 0)
   {
      ...
      if (%barrel !$= "")
      {
         ...
         %turname = %turret.getDatablock().getName();
         if(%turname $= "MobileTurretBase")
         {
