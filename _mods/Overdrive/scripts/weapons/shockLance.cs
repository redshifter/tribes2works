...
datablock ShapeBaseImageData(ShockLanceImage)
{
   ...
   missEnergy = 0;
   hitEnergy  = 0;
   minEnergy  = 0;       // needs to change to be datablock's energy drain for a hit
...
// Overdrive
//--------------------------------------
datablock ShockLanceProjectileData(OverdriveShocker)
{
   directDamage        = 0.45;
   radiusDamageType    = $DamageType::ShockLance;
   kickBackStrength    = 2500;
   ...
   impulse = -1800;
   boltLength = 42.0;
   extension = 42.0;            // script variable indicating distance you can shock people from
   lightningFreq = 25.0;
   lightningDensity = 3.0;
   lightningAmp = 0.25;
   lightningWidth = 0.05;
...
datablock ItemData(ShockLanceOverdrive)
{
   className    = Weapon;
   catagory     = "Spawn Items";
   shapeFile    = "weapon_shocklance.dts";
   image        = ShockLanceOverdriveImage;
   mass         = 1;
   elasticity   = 0.2;
   friction     = 0.6;
   pickupRadius = 2;
	pickUpName   = "a shocklance in overdrive";

   computeCRC = false;
   emap = true;
};

datablock ShapeBaseImageData(ShockLanceOverdriveImage)
{
   classname = WeaponImage;
   shapeFile = "weapon_shocklance.dts";
   item = ShockLanceOverdrive;
   offset = "0 0 0";
   emap = true;

   aura = AuraLance;

   projectile = OverdriveShocker;
