...
function ShapeBaseImageData::onFire(%data, %obj, %slot)
{
   ...
   if(%wpnName $= "DiscImage") {
	   cancel(%obj.discRefillSchedule);
	   %obj.discRefillSchedule = schedule(%data.refillTime, %obj, refillDiscAmmo, %obj);
   }
   else if( %wpnName $= "GrenadeLauncherImage" ) {
	   cancel(%obj.grenRefillSchedule);
	   %obj.grenRefillSchedule = schedule(%data.refillTime, %obj, refillGrenAmmo, %obj);
   }
...
function refillDiscAmmo(%obj) {	
	if( %obj.getState() $= "Dead" ) return; 
	%obj.setInventory(DiscAmmo, 100); 
	serverPlay3D(ItemPickupSound, %obj.getTransform());
}

function refillGrenAmmo(%obj) {	
	if( %obj.getState() $= "Dead" ) return; 
	%obj.setInventory(GrenadeLauncherAmmo, 100); 
	serverPlay3D(ItemPickupSound, %obj.getTransform());
}