      ...
      switch$ (%mode)
      {
         ...
         case "observerFly":
            if(%obj.projectile !$= "")
            {
               ...
               %obj.setFlyMode();
               bottomPrint(%client, "Free Fly Mode.\nPress your \"Throw Grenade\" key to resume player control.", 0, 2);
            }
            else
               Parent::setMode(%data, %obj, %mode, %targetObj);
...
function PracticeCTFGame::gameOver(%game)
{
   ...
   for(%f = 1; %f <= %game.numTeams; %f++)
   {
      ...
      cancel($TeamFlag[%f].lavaEnterThread);
   }
...
function PracticeCTFGame::awardScoreTkDestroy(%game, %cl, %obj)
{
   %cl.tkDestroys++;
...
function PracticeCTFGame::awardScoreStaticShapeDestroy(%game, %cl, %obj)
{
...
function PracticeCTFGame::staticShapeOnRepaired(%game, %obj, %objName)
{
      ...
      else
      {
         ...
         if(%file.openForRead(%filename))
         {
            ...
            for(%i = 0; %i < $DeployablesCount; %i++)
            {
               ...
               // z0dd - ZOD, 5/27/03. Station requires a trigger, needs it set here.
               if($DeployableDataBlockName[%i] $= DeployedStationInventory)
               {
                  %deplObj.trigger = new Trigger() {
                     dataBlock = stationTrigger;
                     polyhedron = "-0.125 0.0 0.1 0.25 0.0 0.0 0.0 -0.7 0.0 0.0 0.0 1.0";
                  };             
                  MissionCleanup.add(%deplObj.trigger);

                  %trans = $DeployableTrans[%i];
                  ...
                  %deplObj.trigger.setTransform(%pos @ " " @ %vRot @ " " @ %vAngle);
                  // associate the trigger with the station
                  %deplObj.trigger.station = %deplObj;
                  %deplObj.trigger.mainObj = %deplObj;
                  %deplObj.trigger.disableObj = %deplObj;
