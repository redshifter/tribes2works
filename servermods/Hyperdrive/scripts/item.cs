...
function Beacon::onUse(%data, %obj) {
	if( VectorLen(%obj.getVelocity()) > 30 ) {
		serverPlay3D(HyperdriveStopSound, %obj.getTransform());
	}
	%obj.setVelocity("0 0 0");
}

datablock EffectProfile(HyperdriveStopEffect)
{
    effectname = "packs/packs.pickupPack";
    minDistance = 50.0;
};

datablock AudioProfile(HyperdriveStopSound)
{
   filename    = "fx/misc/carscreech.wav";
   description = AudioClosest3d;
   effect = HyperdriveStopEffect;
   preload = true;
};


function Bacon::onUse(%data, %obj)
{
